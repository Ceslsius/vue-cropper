...
export default {
  ...
  methods: {
    ...
    changeSize(e) {
      ...
      this.scaling = true
      if (centerBox && !this.checkoutImgAxis(this.x, this.y, scale)) {
        // 缩放后截图框会超出图片，判断是位置问题还是缩放比例问题
        // 先以截图框高度为基准，如果截图框超过了图片则以截图框宽度为基准
        const _scale = scale
        scale = this.cropH / this.trueHeight
        if (this.cropW > this.trueWidth * scale) {
          scale = this.cropW / this.trueWidth
        }
        if (_scale > scale) {
          // 还没有缩放到最小尺寸，是截图框位置错误引起的
          scale = _scale
        }
        let axis = this.getImgAxis(this.x, this.y, scale)
        let cropAxis = this.getCropAxis()
        if (axis.x1 > cropAxis.x1) {
          this.cropOffsertX = axis.x1
        }
        if (axis.x2 < cropAxis.x2) {
          this.cropOffsertX = axis.x2 - this.cropW
        }
        if (axis.y1 >= cropAxis.y1) {
          this.cropOffsertY = axis.x1
        }
        if (axis.y2 <= cropAxis.y2) {
          this.cropOffsertY = axis.y2 - this.cropH
        }
        // 触发截图框移动事件
    ...
    endCrop() {
      ...
      const { width, height } = this.fixed ? this.calculateSize(this.fixedNumber[0], this.fixedNumber[1], minCropW, minCropH, this.cropW, this.cropH) : { width: minCropW, height: minCropH }
      if (width > this.cropW) {
        this.cropW = width
